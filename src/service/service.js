...
function requestInterceptor(config) {
	if (!config.multipart) {
		if (config.method == 'post') {
			//post请求时实现application/x-www-form-urlencoded格式数据传递（FormData)
...
class Service {
	...
	initAsync() {
		...
		return axios
			...
			.then(({ data: { initialized, dev, servers, ...rest } }) => {
				...
				const config =
					serverCount == 1 || isHost(origin)
						? svrs[0]
						: serverCount > 1
						? svrs.find((d) => d.origin.toLowerCase() == origin.toLowerCase())
						: null;
	...
	async UploadFile(file, path, options, query) {
		try {
			...
			const { data } = await this.File.Plugin('FileDownload').post(
				`upload/${path}/${file.name}`,
				formData,
				{ multipart: true, ...options }
			);
			if (data.success) {
	...
	async UploadFile2(file, path, options) {
		try {
			const formData = new FormData();
			formData.append('filedata', file);
			const { data } = await this.File.Plugin('FileDownload').post(
	...
	LayerURL(lyr) {
		...
		const noPlaceHolder = !['{cityorigin}', '{gisorigin}'].some((placeholder) =>
			lyr.url.toLowerCase().includes(placeholder)
		);
		if (!someOrigin && noProtocol && noPlaceHolder) {
			//GIS服务器5.0
			switch (lyr.type) {
				case 'vector-tile':
			...
			return lyr.url
				.replace(/\{cityorigin\}/i, cityorigin)
				.replace(/\{gisorigin\}/i, gisorigin);
		}
	}
